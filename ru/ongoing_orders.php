			...
			<div class="container">
				...
				<div class="orders__items">
					...
					<?php
						...
						$query = "SELECT * FROM orders_ru WHERE user_id = ".$_SESSION['user_id']." AND order_finish = '0' ORDER BY order_id ASC";
						$days_week = ['Monday' => 'Понедельник', 'Tuesday' => 'Вторник', 'Wednesday' => 'Среда','Thursday' => 'Четверг','Friday' => 'Пятница'];
						$day_trns = '';
						...
						if($result){
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								foreach ($array_products as $key => $value) {
									...
									$sort[] = array('product_id' => $value['product_id'], 'product_quantity' => $value['product_quantity'], 'product_name' => $value['product_name'], 
									'product_price' => $value['product_price'], 'product_date' => $value['product_date'], 'product_day' => $day_trns, 'product_finish' => $value['product_finish'], 
									'delivery_address' => $row['delivery_address'], 'order_finish' => $value['order_finish'], 'order_finish' => $value['order_finish'], 'comments' => $row['comments']);
							...
							foreach ($sort as $key => $value) {
									...
									if($value['product_finish'] == 0){
										echo '<div class="orders__item">
											<h2 class="orders__content-title orders__content-title--user">День доставки</h2>
